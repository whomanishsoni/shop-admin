        ...
        if (data.success) {
            ...
            if (product.images && product.images.length > 0) {
                product.images.forEach((image, index) => {
                    const isActive = index === 0; // First image is active by default
                    slidesHTML += `
            ...
            // Initialize Glightbox for magnifier
...
document.getElementById('quickview-add-to-cart-form').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.querySelector('.close-modal').click(); // Close modal silently
        } else {
