                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-4">
                            ...
                            <div id="video-drop-zone" class="border border-2 border-dashed rounded p-5 text-center"
                                ...
                                <p class="mb-0">Drag and drop new video here or click to browse</p>
                            ...
                            <div id="video-preview" class="mt-3 text-center" style="display: none;">
                                <h6>New Video Selected:</h6>
                                <video id="video-element" controls style="max-width: 100%; max-height: 300px;">
                                    <source id="video-source" src="" type="video/mp4">
                                    Your browser does not support the video tag.
                                </video>
                                <br>
                                <button type="button" class="btn btn-sm btn-outline-danger mt-2" id="remove-video">
                                    <i class="fas fa-trash"></i> Remove & Keep Current Video
                                </button>
                                <small class="text-muted d-block mt-2">
                                    Leave checked to replace current video with new upload.
                                </small>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    function uploadFileInChunks(file) {
        ...
        function uploadChunk(chunkIndex) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    uploadedChunks++;
                    // Console log progress instead of progress bar
                    console.log(`Upload progress: ${Math.round((uploadedChunks / totalChunks) * 100)}%`);
    ...
    function mergeChunks(uploadId, fileName, totalChunks) {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert('Video uploaded successfully! You can now update the video.');
            } else {
    ...
    // Handle form submission to include uploaded file path
    $('form').on('submit', function(e) {
        ...
        if (uploadedFilePath) {
            ...
            // Add hidden input with uploaded file path
            $('<input>').attr({
                type: 'hidden',
                name: 'video_path',
