...
<form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col-md-8">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control ckeditor @error('description') is-invalid @enderror"
                                  id="description" name="description" rows="10">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">Existing Product Images</h6>
                </div>
                <div class="card-body">
                    <div class="row">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    @foreach($attributes as $attribute)
                        <div class="mb-3">
                            <label for="attribute_{{ $attribute->id }}" class="form-label">{{ $attribute->display_name }}</label>
                            @php
                                $oldValues = old('attributes.' . $attribute->id, $attributeValues[$attribute->id] ?? []);
                            @endphp
                            @if($attribute->values)
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('meta_keywords') is-invalid @enderror"
                               id="meta_keywords" name="meta_keywords" value="{{ old('meta_keywords', $product->meta_keywords) }}" placeholder="Separate with commas">
                        @error('meta_keywords')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control @error('sku') is-invalid @enderror"
                                   id="sku" name="sku" value="{{ old('sku', $product->sku) }}" placeholder="Leave blank to auto-generate">
                            <button type="button" class="btn btn-outline-secondary" id="generate-sku">Generate SKU</button>
                        </div>
                        @error('sku')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select select2 @error('subcategories') is-invalid @enderror"
                                id="subcategories" name="subcategories[]" multiple>
                            <option value="">Select Subcategories</option>
                            @foreach($subcategories as $subcategory)
                                <option value="{{ $subcategory->id }}" {{ in_array($subcategory->id, old('subcategories', $selectedSubcategories)) ? 'selected' : '' }}>
                                    {{ $subcategory->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select select2 @error('collections') is-invalid @enderror"
                                id="collections" name="collections[]" multiple>
                            <option value="">Select Collections</option>
                            @foreach($collections as $collection)
                                <option value="{{ $collection->id }}" {{ in_array($collection->id, old('collections', $selectedCollections)) ? 'selected' : '' }}>
                                    {{ $collection->name }}
                                </option>
                            @endforeach
                        </select>
...
$(document).ready(function() {
    ...
    if (initialCategory) {
        loadSubcategories(initialCategory, @json(old('subcategories', $selectedSubcategories)));
    }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(key, value) {
                        var sel = ($.inArray(value.id.toString(), selectedSubs.map(String)) !== -1) ? 'selected' : '';
